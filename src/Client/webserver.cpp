...
void handle_shutdown_signal(int signal) {
    if (signal == SIGINT || signal == SIGTERM) {
        std::cout
            << "WEB SERVER: Shutdown signal received. Stopping server...\n\n";
...
void* create_client_thread(void* client_socket_ptr) {
    ...
    if (request_bytes < 0) {
        std::cerr << "WEB SERVER: Error reading request\n";
        close(socket);
        return nullptr;
    }
    ...
    if (method_end == std::string::npos || path_end == std::string::npos) {
        std::cerr << "WEB SERVER: Malformed request\n";
    ...
    if (http_version != "HTTP/1.1" && http_version != "HTTP/1.0") {
        std::cerr << "WEB SERVER: Invalid HTTP version: " << http_version
                  << "\n";
        const std::string response = CreateHTTPResponse(HTTP_BAD);
        write(socket, response.c_str(), response.size());
        close(socket);
    ...
    //std::cout << "WEB SERVER: File Requested: " << requested_file << "\n";
    ...
    std::string file_type = requested_file.substr(requested_file.find('.'));
    ...
    for (const std::string& filename : forbidden_files) {
        if (requested_file.substr(1, requested_file.size()) == filename) {
            std::cerr << "WEB SERVER: Forbidden file requested: "
                      << requested_file << "\n";
    ...
    if (strncmp(resolved_path.data(), current_dir.data(),
        ...
        std::cerr << "WEB SERVER: Forbidden: File outside current directory\n";
    ...
    if (file == nullptr) {
        std::cerr << "WEB SERVER: File not found\n";
    ...
    std::string response = CreateHTTPResponse(HTTP_OKAY, mime_type);
    write(socket, response.c_str(), response.size());

    while (fgets(buffer.data(), BUFFER_SIZE, file) != nullptr) {
        if (write(socket, buffer.data(), strlen(buffer.data())) == -1) {
            std::cerr << "WEB SERVER: Error sending data\n";
            break;
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    if (getaddrinfo(nullptr, port.c_str(), &hints, &servinfo) != 0) {
        std::cerr << "WEB SERVER: getaddrinfo error" << std::endl;
        return 1;
    }

    std::cout << "WEB SERVER: Starting web server on port: " << port << "\n\n";
    ...
    for (p = servinfo; p != nullptr; p = p->ai_next) {
        ...
        if (setsockopt(server_sd, SOL_SOCKET, SO_REUSEADDR, (char*)&on,
                       sizeof(on)) == -1) {
            std::cerr << "WEB SERVER: setsockopt failed" << std::endl;
            close(server_sd);
            continue;  //Try the next one
        }
    ...
    if (p == nullptr) {
        std::cerr << "WEB SERVER: Failed to bind the socket" << "\n";
    ...
    if (listen(server_sd, CONNECTIONS) == -1) {
        std::cerr << "WEB SERVER: Listen error" << "\n";
        close(server_sd);
        return -1;
    }
    ...
    while (server_running) {
        ...
        *new_sd =
        ...
        if (*new_sd == -1) {
            ...
            std::cerr << "WEB SERVER: Accept error" << "\n";
            delete new_sd;  //Clean up memory if accept fails
            continue;
        }

        // std::cout << "WEB SERVER: New connection\n";  //<< *new_sd << "\n";
        ...
        if (pthread_create(&thread, nullptr, create_client_thread, new_sd) >
            0) {
            std::cerr << "WEB SERVER: Thread not created. \n";
            close(*new_sd);
